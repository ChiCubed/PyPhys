...
class Circle(object):
	def __init__(self, center, r, mass = 1, dynamic = True):
		self.center = center
		self.r = r
		self.rs = self.r**2
		self.mass = mass
	...
	@classmethod
	def from_points(self, p1, p2, mass = 1, dynamic = True):
		self.__init__(p1, (p2 - p1).magnitude(), mass, dynamic)

	@classmethod
	def from_squaredradius(self, center, rs, mass = 1, dynamic = True):
		self.center = center
		self.r = sqrt(rs)
		self.rs = rs
		self.mass = mass
	...
	def calculate_density(self):
		# Calculates density
		# in kg / m^2
		self.density = self.mass / (self.r * self.r * pi)
